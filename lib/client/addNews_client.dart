...
class addNewsClient {
  ...
  static final String locationEndpoint = '/api/newsloc';
  ...
  static Future<Response> updateLokasi(id, String lokasi) async {
    try {
      var response = await put(Uri.http(url, '$locationEndpoint/${id}'),
          body: jsonEncode({"lokasi": lokasi}),
          headers: {"Content-Type": "application/json"});
      if (response.statusCode != 200) throw Exception(response.body);
